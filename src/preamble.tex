% SPDX-License-Identifier: CC-BY-NC-SA-4.0
%
% preamble.tex -- document configuration
% Copyright (C) 2024--2025  Jacob Koziej <jacobkoziej@gmail.com>

\usepackage{geometry}
\geometry{
	marginpar=1.8cm,
	paper=b5paper,
}

\usepackage{bookmark}
\usepackage{booktabs}
\usepackage{emoji}
\usepackage{enumitem}
\usepackage{fancyhdr}
\usepackage{fontawesome}
\usepackage{graphicx}
\usepackage{listings}
\usepackage{longtable}
\usepackage{marginnote}
\usepackage{parskip}
\usepackage{ragged2e}
\usepackage{siunitx}
\usepackage{subcaption}
\usepackage{tikz-among-us}
\usepackage{tikz}
\usepackage{titling}
\usepackage{wrapfig}
\usepackage{xurl}

\usepackage{mathtools}
\usepackage{unicode-math}
\unimathsetup{
	math-style=ISO,
	warnings-off={
		mathtools-colon,
		mathtools-overbracket,
	},
}

\usepackage[cache=false]{minted}
\setminted{
	breaklines,
	linenos,
	obeytabs,
}

\input{git-hash}

\author{Jacob~Koziej}

\pagestyle{fancy}
\setcounter{secnumdepth}{0}

\fancyhead{}
\fancyhead[L]{%
	\small\slshape%
	ECE 210: MATLAB Seminar -- Signals \& Systems%
}
\fancyhead[R]{%
	\scriptsize\ttfamily%
	\faCodeFork\,\gitHash\gitDirty%
}
\fancyfoot{}
\fancyfoot[L]{%
	\scriptsize\slshape%
	Copyright \copyright\ 2024--2025 \theauthor%
	~---~%
	\ttfamily%
	\href{https://creativecommons.org/licenses/by-nc-sa/4.0/}%
	{CC BY-NC-SA 4.0}%
}
\fancyfoot[R]{%
	\scriptsize\ttfamily%
	\href{https://github.com/jacobkoziej/jk-ece210}%
	{github.com/jacobkoziej/jk-ece210}%
}
\RenewDocumentCommand{\footrule}{}{\rule{\headwidth}{\headrulewidth}}
\RenewDocumentCommand{\footruleskip}{}{1pt}

\NewDocumentCommand{\aside}{m}{%
	\marginnote{%
		\scriptsize%
		\raggedright%
		\textsl{\textbf{Aside---}}%
		\vspace{-1em}%
		\par\noindent%
		#1%
	}
}

\NewDocumentCommand{\C}{}{{\symbb{C}}}

\NewDocumentCommand{\cpi}{}{\symrm{\pi}}

\NewDocumentCommand{\e}{}{\symrm{e}}

\NewDocumentCommand{\hint}{m}{\textbf{Hint:}~{#1}}

\NewDocumentCommand{\footurl}{m}{\footnote{\url{#1}}}

\NewDocumentCommand{\im}{}{\symrm{j}}

\NewDocumentCommand{\mCommand}{om}{%
	\IfValueTF{#1} {%
		\href{#1}{\mintinline{matlab}{#2}}%
	} {%
		\mintinline{matlab}{#2}%
	}%
}

\NewDocumentCommand{\mCommandCustom}{mm}{%
	\begin{minipage}{\textwidth}%
	\inputminted{matlab}{#1.m}%
	\vspace{1em}%
	\includegraphics[width=\textwidth]{#1.#2}%
	\end{minipage}%
}

\NewDocumentCommand{\mCommandGraphic}{m}{%
	\begin{minipage}{\textwidth}%
	\inputminted{matlab}{#1.m}%
	\vspace{1em}%
	\includegraphics[width=\textwidth]{#1.eps}%
	\end{minipage}%
}

\NewDocumentCommand{\mCommandGraphicSplit}{m}{%
	\inputminted{matlab}{#1.m}%
	\includegraphics[width=\textwidth]{#1.eps}%
}

\NewDocumentCommand{\mCommandOutput}{m}{%
	\begin{minipage}{\textwidth}%
	\inputminted{matlab}{#1.m}%
	\vspace{1em}%
	\footnoterule%
	\inputminted{text}{#1.txt}%
	\end{minipage}%
}

\NewDocumentCommand{\mat}{m}{\symbfit{#1}}

\NewDocumentCommand{\matrixField}{mmm}{%
	\symrm{M}_{#1 \times #2} (\symbb{#3})%
}

\NewDocumentCommand{\renderTitle}{}{%
	{\noindent\LARGE\scshape\thetitle\vspace{1ex}}%
	\pdfbookmark[1]{\thetitle}{title}%
}

\NewDocumentCommand{\T}{}{{\symrm{T}}}

\NewDocumentCommand{\vocab}{m}{\textsl{\textbf{#1}}}

\NewDocumentCommand{\Z}{}{{\symbb{Z}}}
